...
module.exports = async (fastify, _options) => {
                ...
                if (existingTrack) {
                  const records = await Promise.all(
                    users.map(async user => {
                      const record = await fastify.writeDb('messages').insert({
                  ...
                  const createdRecords = await fastify
        ...
        if (existingTrack) {
          ...
          const records = await Promise.all(
            users.map(async user => {
              ...
              sendNotificationOnReceivedSong(currentUserId, user.id).catch();
              return record;
            }),
          );
          if (records.length > 0) {
  ...
  function insertIntoTracksTable(existingTrack) {
    fastify
      .readDb('tracks')
      .select(['track_id', 'platform', 'id'])
      .where({ track_id: existingTrack.id, platform: 'spotify' })
      .first()
      .then(alreadyTrack => {
        if (!alreadyTrack) {
          const newTrack = {};
