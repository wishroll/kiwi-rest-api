...
module.exports = async (fastify, _options) => {
    const { sendNotificationOnReceivedSong, sendPushNotificationOnReceivedFriendRequest, sendNotificationOnCreatedRating } = require('../../services/notifications/notifications')
    fastify.post('/v1/notifications/messages/test', { onRequest: [fastify.authenticate] }, async (req, res) => {
    ...
    fastify.post('/messages/ratings/test', { onRequest: [fastify.authenticate] }, async (req, res) => {
        const messageId = '51';
