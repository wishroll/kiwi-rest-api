...
export const ratedMessagesQuery = {
  type: 'object',
  properties: {
    limit: { type: 'integer', minimum: 1 },
    lastId: { type: 'integer', description: 'Id of last message in previous iteration' },
  ...
  required: ['limit', 'liked'],
} as const;

export const ratedMessagesSchema = {
  description: 'Return a list of song messages that have been rated by current user.',
  tags: ['Messages', 'Rated'],
  summary: "Returns an array of a user's rated song messages",
  query: ratedMessagesQuery,
  headers: {
    type: 'object',
    properties: {
      Authorization: { type: 'string', description: 'The token used for authentication' },
